...
class MztTransformDatasetReader(Iterable):
    ...
    def _read_labels(self):
        ...
        for image_path in tqdm(self._image_paths, 'The labels reading'):
            ...
            with open(labels_path) as labels_file:
                ...
                labels_data = [float(f) for f in labels_str.split(' ')]
                labels_data = [(x, y) for x, y in zip(labels_data[::2], labels_data[1::2])]
                self._labels.append(labels_data)

    def __iter__(self):
        return self

    def __next__(self) -> Tuple[np.ndarray, List[Tuple[float, float]]]:
        self._iter_index += 1
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray, List[Tuple[float, float]]]:
